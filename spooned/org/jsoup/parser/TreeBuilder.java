...
abstract class TreeBuilder {
    ...
    protected org.jsoup.nodes.Element currentElement() {
        int size = stack.size();
        return size > 0 ? stack.get(((_arithmeticOperatorHotSpot106.is("PLUS") ? ((size + 1))) : (_arithmeticOperatorHotSpot106.is("MINUS") ? ((size - 1))) : (_arithmeticOperatorHotSpot106.is("MUL") ? ((size * 1))) : (_arithmeticOperatorHotSpot106.is("DIV") ? ((size / 1))))) : null;
    }

    private static final metamutator.Selector _arithmeticOperatorHotSpot106 = metamutator.Selector.of(106,new String[]{"MINUS","PLUS","MUL","DIV"}).in(org.jsoup.parser.TreeBuilder.class).id("_arithmeticOperatorHotSpot106");
}
