...
public class Parser {
    ...
    public static org.jsoup.nodes.Document parseBodyFragment(java.lang.String bodyHtml, java.lang.String baseUri) {
        ...
        for (int i = ((_arithmeticOperatorHotSpot88.is("PLUS") ? ((nodes.length) + 1)) : (_arithmeticOperatorHotSpot88.is("MINUS") ? ((nodes.length) - 1)) : (_arithmeticOperatorHotSpot88.is("MUL") ? ((nodes.length) * 1)) : (_arithmeticOperatorHotSpot88.is("DIV") ? ((nodes.length) / 1))) ; i > 0 ; i--) {
            nodes[i].remove();
        }
    ...
    private static final metamutator.Selector _arithmeticOperatorHotSpot88 = metamutator.Selector.of(88,new String[]{"MINUS","PLUS","MUL","DIV"}).in(org.jsoup.parser.Parser.class).id("_arithmeticOperatorHotSpot88");
}
