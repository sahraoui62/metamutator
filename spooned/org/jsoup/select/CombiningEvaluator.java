...
abstract class CombiningEvaluator extends org.jsoup.select.Evaluator {
    ...
    org.jsoup.select.Evaluator rightMostEvaluator() {
        return (num) > 0 ? evaluators.get(((_arithmeticOperatorHotSpot112.is("PLUS") ? (((num) + 1))) : (_arithmeticOperatorHotSpot112.is("MINUS") ? (((num) - 1))) : (_arithmeticOperatorHotSpot112.is("MUL") ? (((num) * 1))) : (_arithmeticOperatorHotSpot112.is("DIV") ? (((num) / 1))))) : null;
    }

    void replaceRightMostEvaluator(org.jsoup.select.Evaluator replacement) {
        evaluators.set(((_arithmeticOperatorHotSpot113.is("PLUS") ? (((num) + 1))) : (_arithmeticOperatorHotSpot113.is("MINUS") ? (((num) - 1))) : (_arithmeticOperatorHotSpot113.is("MUL") ? (((num) * 1))) : (_arithmeticOperatorHotSpot113.is("DIV") ? (((num) / 1)))), replacement);
    }
    ...
    private static final metamutator.Selector _arithmeticOperatorHotSpot112 = metamutator.Selector.of(112,new String[]{"MINUS","PLUS","MUL","DIV"}).in(org.jsoup.select.CombiningEvaluator.class).id("_arithmeticOperatorHotSpot112");

    private static final metamutator.Selector _arithmeticOperatorHotSpot113 = metamutator.Selector.of(113,new String[]{"MINUS","PLUS","MUL","DIV"}).in(org.jsoup.select.CombiningEvaluator.class).id("_arithmeticOperatorHotSpot113");
}
